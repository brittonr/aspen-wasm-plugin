...
pub fn guest_dispatch_function(function_call: FunctionCall) -> Result<Vec<u8>> {
    ...
    let store = store
        .deref_mut()
        .as_mut()
        .ok_or(HyperlightGuestError::new(ErrorCode::GuestError, "No wasm store available".to_string()))?;
    let instance = CUR_INSTANCE.lock();
    let instance = instance
        .deref()
        .as_ref()
        .ok_or(HyperlightGuestError::new(ErrorCode::GuestError, "No wasm instance available".to_string()))?;
    ...
    marshal::free_return_value_allocations(&mut *store, &|ctx, name| instance.get_export(ctx, name))?;

    let func = instance
        .get_func(&mut *store, &function_call.function_name)
        .ok_or(HyperlightGuestError::new(ErrorCode::GuestError, "Function not found".to_string()))?;

    let mut w_params = vec![];
    for f_param in (function_call.parameters).as_ref().unwrap_or(&vec![]).iter() {
        w_params.push(marshal::hl_param_to_val(&mut *store, |ctx, name| instance.get_export(ctx, name), f_param)?);
    }
...
fn init_wasm_runtime() -> Result<Vec<u8>> {
    ...
    let hostfuncs = hostfuncs::get_host_function_details().host_functions.unwrap_or_default();
    for hostfunc in hostfuncs.iter() {
        ...
        linker.func_new(
            ...
            move |c, ps, rs| {
                hostfuncs::call(&captured, c, ps, rs).map_err(|e| wasmtime::Error::msg(format!("{:?}", e)))
            },
        )?;
    }
...
fn load_wasm_module(function_call: &FunctionCall) -> Result<Vec<u8>> {
    if let (ParameterValue::VecBytes(ref wasm_bytes), ParameterValue::Int(ref _len), Some(ref engine)) = (
