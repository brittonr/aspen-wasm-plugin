...
// general todos:
// - split out the general guest codegen (to do an `impl Imports for Host {}`) vs the
//   wasmtime-specific codegen
// - once that is done it will be easy to support resources exported from the guest properly. (the
//   current issue is that since the host-interaction code is fused with the wasmtime-interface
//   code, it is impossible to come up with an <I: Imports> to instantiate the `Resources` struct
//   with.)

use hyperlight_component_util::emit::FnName;
use hyperlight_component_util::emit::ResolvedBoundVar;
use hyperlight_component_util::emit::State;
use hyperlight_component_util::emit::WitName;
use hyperlight_component_util::emit::kebab_to_fn;
use hyperlight_component_util::emit::kebab_to_namespace;
use hyperlight_component_util::emit::kebab_to_type;
use hyperlight_component_util::emit::kebab_to_var;
use hyperlight_component_util::emit::split_wit_name;
use hyperlight_component_util::etypes::Component;
use hyperlight_component_util::etypes::Defined;
use hyperlight_component_util::etypes::ExternDecl;
use hyperlight_component_util::etypes::ExternDesc;
use hyperlight_component_util::etypes::Handleable;
use hyperlight_component_util::etypes::Instance;
use hyperlight_component_util::etypes::Tyvar;
use hyperlight_component_util::etypes::{self};
use hyperlight_component_util::hl::emit_fn_hl_name;
use hyperlight_component_util::hl::emit_hl_marshal_param;
use hyperlight_component_util::hl::emit_hl_marshal_result;
use hyperlight_component_util::hl::emit_hl_unmarshal_param;
use hyperlight_component_util::hl::emit_hl_unmarshal_result;
use hyperlight_component_util::resource;
use hyperlight_component_util::rtypes;
...
fn emit_import_extern_decl<'b>(s: &mut State<'_, 'b>, depth: u32, ed: &ExternDecl<'b>) -> TokenStream {
    match &ed.desc {
        ...
        ExternDesc::Func(ft) => {
            ...
            let pts = ft.params.iter().map(|p| rtypes::emit_value(s, &p.ty)).collect::<Vec<_>>();
...
// call.
fn emit_export_extern_decl<'b>(s: &mut State<'_, 'b>, path: Vec<String>, ed: &ExternDecl<'b>) -> TokenStream {
    match &ed.desc {
        ExternDesc::CoreModule(_) => panic!("core module (im/ex)ports are not supported"),
        ExternDesc::Func(ft) => {
            let fname = emit_fn_hl_name(s, ed.kebab_name);
            ...
            let pts = ft
                .params
                .iter()
                .map(|_| quote! { ::hyperlight_common::flatbuffer_wrappers::function_types::ParameterType::VecBytes })
                .collect::<Vec<_>>();
            let pwts = ft.params.iter().map(|p| rtypes::emit_value(s, &p.ty)).collect::<Vec<_>>();
...
// depth: how many instances deep (from the root component) this is,
// used to keep track of which linker instance to register on
fn emit_import_instance<'b>(s: &mut State<'_, 'b>, wn: WitName, depth: u32, it: &Instance<'b>) -> TokenStream {
    ...
    let imports = it.exports.iter().map(|ed| emit_import_extern_decl(&mut s, depth, ed)).collect::<Vec<_>>();
    quote! { #(#imports)* }
}
...
// definition may be found, used to locate the wasmtime function to
// call.
fn emit_export_instance<'b>(s: &mut State<'_, 'b>, wn: WitName, path: Vec<String>, it: &Instance<'b>) -> TokenStream {
    let mut s = s.with_cursor(wn.namespace_idents());
    s.cur_helper_mod = Some(kebab_to_namespace(wn.name));
    s.cur_trait = Some(kebab_to_type(wn.name));
    let exports = it.exports.iter().map(|ed| emit_export_extern_decl(&mut s, path.clone(), ed)).collect::<Vec<_>>();
    quote! { #(#exports)* }
}

// Emit:
// - a resource table for all resource exported by this component, to keep track of resources sent
//   to the host
...
fn emit_component<'b>(s: &mut State<'_, 'b>, wn: WitName, ct: &Component<'b>) -> TokenStream {
    ...
    s.root_mod.items.extend(s.bound_vars.iter().enumerate().map(|(i, _)| {
    ...
    let imports = ct.imports.iter().map(|ed| emit_import_extern_decl(&mut s, 0, ed)).collect::<Vec<_>>();
    s.var_offset = 0;
