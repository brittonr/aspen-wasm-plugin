...
//!
//! This module implements a clear memory ownership model for both guest function calls and host
//! function calls:
//!
//! ## Guest Function Parameters (Host → Guest)
//! - When calling guest functions with String or VecBytes parameters, the host allocates memory in
//!   the guest's memory space and passes pointers to the guest.
//! - **The guest owns these allocations and must free them** when no longer needed using the `free`
//!   function exported from the guest module.
//!
//! ## Guest Function Return Values (Guest → Host)  
//! - When guest functions return String or VecBytes values, the guest allocates memory in its own
//!   memory space and returns pointers to the host using the malloc corresponding to its exported
//!   free.
//! - **The host takes ownership of these allocations and will free them** on the next VM entry to
...
fn malloc<C: AsContextMut>(
    ...
    let malloc = get_export(&mut *ctx, "malloc")
        ...
        .ok_or(HyperlightGuestError::new(ErrorCode::GuestError, "malloc function not exported".to_string()))?;
    let addr = malloc.typed::<i32, i32>(&mut *ctx)?.call(&mut *ctx, len as i32)?;
    Ok(addr)
}

#[instrument(skip_all, level = "Trace")]
fn free<C: AsContextMut>(ctx: &mut C, get_export: &impl Fn(&mut C, &str) -> Option<Extern>, addr: i32) -> Result<()> {
    let free = get_export(&mut *ctx, "free")
        .and_then(Extern::into_func)
        .ok_or(HyperlightGuestError::new(ErrorCode::GuestError, "free function not exported".to_string()))?;
...
fn write<C: AsContextMut>(
    ...
    let memory = get_export(&mut *ctx, "memory")
        ...
        .ok_or(HyperlightGuestError::new(ErrorCode::GuestError, "memory not exported".to_string()))?;
    memory
        .write(&mut *ctx, addr as usize, bytes)
        .map_err(|e| HyperlightGuestError::new(ErrorCode::GuestError, format!("error writing to memory: {}", e)))?;
    Ok(())
}
...
fn read<C: AsContextMut>(
    ...
    let memory = get_export(&mut *ctx, "memory")
        ...
        .ok_or(HyperlightGuestError::new(ErrorCode::GuestError, "memory not exported".to_string()))?;
    memory
        .read(&mut *ctx, addr as usize, bytes)
        .map_err(|e| HyperlightGuestError::new(ErrorCode::GuestError, format!("error reading from memory: {}", e)))?;
    Ok(())
}
...
fn read_cstr<C: AsContextMut>(
    ...
    loop {
        memory.read(&mut *ctx, addr as usize, &mut byte).map_err(|e| {
            HyperlightGuestError::new(ErrorCode::GuestError, format!("error reading from memory: {}", e))
        })?;
        if byte[0] == 0 {
            break;
        }
    ...
    CString::new(string)
        .map_err(|e| HyperlightGuestError::new(ErrorCode::GuestError, format!("invalid c string in memory: {}", e)))
}
...
pub fn val_to_hl_result<C: AsContextMut>(
    ...
    match (rt, rvs[0]) {
        ...
        (ReturnType::String, Val::I32(p)) => {
            ...
            Ok(get_flatbuffer_result::<&str>(read_cstr(&mut ctx, &get_export, p)?.to_str().map_err(|e| {
        ...
        (_, _) => Err(HyperlightGuestError::new(
            ErrorCode::GuestError,
            format!("Hyperlight/wasm function return type combination unsupported: {:?} / {:?}", rt, rvs[0]),
        )),
    }
}
...
pub fn val_to_hl_param<'a, C: AsContextMut>(
    ...
    match (pt, v) {
        ...
        (ParameterType::String, Val::I32(p)) => {
            Some(ParameterValue::String(read_cstr(ctx, &get_export, *p).unwrap().to_str().unwrap().to_string()))
        }
        ...
        (_, _) => panic!("Host function return type combination unsupported: {:?} / {:?}", pt, v),
    }
}
