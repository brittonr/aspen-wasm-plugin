...
pub(crate) fn call<T>(d: &HostFunctionDefinition, mut c: Caller<'_, T>, ps: &[Val], rs: &mut [Val]) -> Result<()> {
    let params = d
        .parameter_types
        .iter()
        .flatten()
        .scan((ps.iter(), None), |s, t| marshal::val_to_hl_param(&mut c, |c, n| c.get_export(n), s, t))
        .collect();

    let rv = call_host_function::<ReturnValue>(&d.function_name, Some(params), d.return_type)
        .expect("Host function call failed");

    assert!(return_type_from_val(&rv) == d.return_type, "Host function return type mismatch");

    if rs.is_empty() {
        assert!(d.return_type == ReturnType::Void, "Host function return type mismatch");
        return Ok(());
    }
