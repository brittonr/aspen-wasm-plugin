...
fn page_fault_handler(
    ...
    if (error_code & 0x1) == 0x0 && page_fault_address >= 0x100_0000_0000u64 {
        unsafe {
            ...
            paging::map_region(phys_page, virt_base, hyperlight_guest_bin::OS_PAGE_SIZE as u64);
...
pub extern "C" fn wasmtime_mmap_remap(addr: *mut u8, size: usize, prot_flags: u32) -> i32 {
    if size > 0x100_0000_0000 {
        panic!("wasmtime_mmap_remap {:x} {:x} {:x}", addr as usize, size, prot_flags);
    }
    0
}
...
type wasmtime_trap_handler_t = extern "C" fn(ip: usize, fp: usize, has_faulting_addr: bool, faulting_addr: usize);
...
#[no_mangle]
pub extern "C" fn wasmtime_memory_image_map_at(_image: *mut c_void, _addr: *mut u8, _len: usize) -> i32 {
...
impl wasmtime::CustomCodeMemory for WasmtimeCodeMemory {
    ...
    fn publish_executable(&self, _ptr: *const u8, _len: usize) -> core::result::Result<(), wasmtime::Error> {
        Ok(())
    }
    fn unpublish_executable(&self, _ptr: *const u8, _len: usize) -> core::result::Result<(), wasmtime::Error> {
        Ok(())
    }
}
