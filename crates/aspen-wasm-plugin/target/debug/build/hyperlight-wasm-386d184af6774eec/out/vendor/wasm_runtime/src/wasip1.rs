...
pub(crate) fn register_handlers<T: 'static>(linker: &mut Linker<T>) -> Result<()> {
    ...
    linker.func_wrap(
        ...
        |mut ctx: Caller<'_, T>, fd: i32, iovs: i32, iovs_len: i32, retptr: i32| {
            ...
            for i in 0..iovs_len as usize {
                ...
                memory.read(&mut ctx, buf as usize, &mut string_bytes).unwrap();
            ...
            memory.write(&mut ctx, retptr, &total_written.to_le_bytes()).unwrap();
            0
        },
    )?;
    ...
    linker.func_wrap("wasi_snapshot_preview1", "random_get", |buff: i32, len: i32| -> i32 {
        panic!("random_get called for {} with len {}", buff, len);
    })?;
    linker.func_wrap("wasi_snapshot_preview1", "fd_fdstat_get", |mut ctx: Caller<'_, T>, fd: i32, retptr: i32| {
        if fd != 1 {
            return -1;
        }
        if let Some(()) = (|| {
            ...
            memory.write(&mut ctx, retptr, &(4_u16).to_le_bytes()).unwrap();
            // offset 2 is fdstat.fs_flags; 0 is no particular flags
            memory.write(&mut ctx, retptr + 2, &(0_u16).to_le_bytes()).unwrap();
            // offset 8 fdstat.fs_rights_base; 0b100011 is read/write/seek
            memory.write(&mut ctx, retptr + 8, &(0b100011_u64).to_le_bytes()).unwrap();
            // offset 8 fdstat.fs_rights_inheriting; 0b100011 is read/write/seek
            memory.write(&mut ctx, retptr + 16, &(0b100011_u64).to_le_bytes()).unwrap();
            Some(())
        })() {
            0
        } else {
