...
fn load_component_common(engine: &Engine, component: Component) -> Result<()> {
    ...
    let instance = (*CUR_LINKER.lock()).as_ref().unwrap().instantiate(&mut store, &component)?;
...
fn load_wasm_module_phys(function_call: &FunctionCall) -> Result<Vec<u8>> {
    if let (ParameterValue::ULong(ref phys), ParameterValue::ULong(ref len), Some(ref engine)) = (
        ...
        let component = unsafe { Component::deserialize_raw(engine, platform::map_buffer(*phys, *len))? };
        load_component_common(engine, component)?;
...
pub fn guest_dispatch_function(function_call: FunctionCall) -> Result<Vec<u8>> {
    Err(HyperlightGuestError::new(ErrorCode::GuestFunctionNotFound, function_call.function_name.clone()))
}
